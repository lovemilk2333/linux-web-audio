...
Below three quarters of the target the shortfall is weighted harder, so
reserve is rebuilt at the one-frame-per-boundary cap before the buffer gets
near empty. A repeat that finds the current render block already full is
deferred into the next block rather than dropped — otherwise boundaries that
land on a quantum edge silently refuse to correct, which left a 20 ms target
dipping to a few milliseconds and clicking. An early refill at about one
render quantum (~4 ms), with a short dwell and a one-second cooldown, is only
a safety net: it does not replace the corrector, and it is kept far below
ordinary burst jitter so it cannot thrash the way a level threshold near the
setpoint did.
